...
/**
 * @description Fetches aggregated summary data for the GSTR-3B report, excluding returned quantities.
 */
export async function getGstr3bReportData({
  ...
  try {
    ...
    // 1. Fetch RAW Sales Data (Net of returns)
    // ✅ FIX: (si.quantity - si.return_quantity)
    const salesQuery = db.prepare(`
      SELECT 
        ...
        si.rate, si.discount, COALESCE(si.gst_rate, p.gst_rate, 0) as gst_rate
      FROM sales s
    ...
    // 2. Fetch RAW Purchases Data (ITC)
    ...
    const gstr3b = {
      outward_supplies: {
        os_tax: { txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        os_nil_exmp: { txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        is_rc: { txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
      },
      itc_eligible: {
        itc_avl: { txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
      },
    };
...
function _calculateItemTax(item, isInclusive) {
  ...
  let taxableValue = isInclusive
    ? baseValue / (1 + gstRate / 100.0)
    : baseValue;
  let taxAmount = isInclusive
    ? baseValue - taxableValue
    : taxableValue * (gstRate / 100.0);
