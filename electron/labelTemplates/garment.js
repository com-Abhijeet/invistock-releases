...
const garmentTemplates = {
  gar_size_circle: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${10 * scale}px; font-weight: 600; width: 80%; overflow: hidden; flex-shrink: 0; }
        .btm { display: flex; justify-content: space-between; align-items: flex-end; border-top: 1px dotted #ccc; padding-top: ${2 * scale}px; flex-grow: 1; min-height: 0; gap: ${4 * scale}px; }
        .bc-box { flex-grow: 1; display: flex; align-items: center; height: 100%; min-height: 0; width: 60%; }
        ...
        <div class="box">
           ...
           <div class="btm">
              ...
              <div style="text-align:right; flex-shrink: 0;">
                 ...
                 ${encoded ? `<div style="font-size:${6 * scale}px; color:#aaa;">${encoded}</div>` : ""}
              </div>
           </div>
        </div>
        ${BRANDING_HTML}
  ...
  gar_boutique: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .in { font-size: ${9 * scale}px; color: #444; margin: ${1 * scale}px 0; overflow: hidden; width: 100%; }
  ...
  gar_grid: (item, shop, barcode, width, height = 25) => {
    const { mainPrice, encoded } = getPriceDetails(item);
    const scale = Math.max(0.5, Math.min(1.2, height / 25));
    const batchTag = item.batch_number || item.batch_no || item.size || "-";
    return `
      <style>
        ${getBaseStyle(width, height)}
        ...
        .bc-cell { grid-column: span 2; text-align: center; padding: ${2 * scale}px; border-bottom: none !important; flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .bc-cell img { width: 100%; max-height: 100%; object-fit: fill; }
  ...
  gar_slim: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${9 * scale}px; font-weight: 600; text-align: center; line-height: 1.1; width: 100%; overflow: hidden; }
        .bc { flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
      ...
      <div class="wrapper">
        <div class="sl">
           <div class="sl-l">${item.batch_number || item.batch_no || item.size || "STD"}</div>
           <div class="sl-r">
              <div class="nm">${nameHTML}</div>
              <div class="bc"><img src="${barcode}" /></div>
              <div class="pr">₹${mainPrice}</div>
  ...
  gar_kids: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .kn { font-size: ${10 * scale}px; font-weight: 700; line-height: 1; width: 100%; }
        .bc { flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
