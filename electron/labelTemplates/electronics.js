...
const electronicsTemplates = {
  ele_spec: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .tt { font-size: ${9 * scale}px; font-weight: 700; border-bottom: 1px solid #eee; margin-bottom: ${2 * scale}px; line-height: 1.1; width: 100%; }
        ...
        .bc { flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; margin: ${1 * scale}px 0; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="es">
           <div class="tt">${nameHTML}</div>
           <div class="rw"><span>Brand</span><b>${item.brand || "-"}</b></div>
           <div class="rw" style="align-items:center;">
              ${encoded ? `<span style="font-size:${6 * scale}px;">${encoded}</span>` : ""}
  ...
  ele_dark: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${9 * scale}px; font-weight: 700; margin-bottom: ${2 * scale}px; line-height: 1.1; width: 100%; }
        .bc-box { background: #fff; padding: ${2 * scale}px; border-radius: ${2 * scale}px; flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .bc-box img { width: 100%; max-height: 100%; object-fit: fill; }
  ...
  ele_warranty: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-weight: 700; font-size: ${9 * scale}px; width: 100%; }
        .w-lbl { font-size: ${7 * scale}px; background: #ddd; display: inline-block; padding: 0 ${2 * scale}px; align-self: flex-start; }
        .rw { display: flex; justify-content: space-between; align-items: center; flex-grow: 1; min-height: 0; gap: ${4 * scale}px; }
        .bc { width: 65%; height: 100%; display: flex; align-items: center; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
        .pr { font-weight: 800; font-size: ${12 * scale}px; flex-shrink: 0; }
      </style>
      <div class="wrapper">
        <div class="wr">
           <div class="nm">${nameHTML}</div>
  ...
  ele_mobile: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${9 * scale}px; font-weight: 600; width: 100%; overflow: hidden; }
  ...
  ele_serial: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${8 * scale}px; font-weight: 600; line-height: 1.1; overflow: hidden; width: 100%; }
        .bc { flex-grow: 1; display: flex; align-items: center; min-height: 0; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="sr">
           ...
           <div class="r">
              ${encoded ? `<div style="font-size:${7 * scale}px; color:#aaa;">${encoded}</div>` : ""}
  ...
  ele_box: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${11 * scale}px; font-weight: 800; line-height: 1.1; margin-bottom: ${2 * scale}px; width: 100%; }
        .specs { font-size: ${8 * scale}px; color: #555; }
        .bc { flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; margin-top: ${2 * scale}px; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="bx">
  ...
  ele_comp: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ${getBaseStyle(width, height)}
        .cp { font-size: ${8 * scale}px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #ddd; padding: ${1 * scale}px; flex-grow: 1; gap: ${4 * scale}px; }
      </style>
      <div class="wrapper">
        <div class="cp">
           ...
           <img src="${barcode}" style="height: 100%; max-height: 100%; width: 35%; object-fit: fill;" />
        </div>
      </div>
    `;
  },
  ...
  ele_gaming: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .gn { font-weight: 700; font-size: ${10 * scale}px; color: #0f0; text-transform: uppercase; width: 100%; }
        .gb { background: #fff; padding: ${2 * scale}px; margin: ${2 * scale}px 0; border-radius: ${2 * scale}px; flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .gb img { width: 100%; max-height: 100%; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="gm">
           <div class="gn">${nameHTML}</div>
