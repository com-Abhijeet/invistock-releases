  ...
  med_dose: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .mn { font-size: ${10 * scale}px; font-weight: 700; line-height: 1.1; margin: ${2 * scale}px 0; flex-shrink: 0; width: 100%; }
        ...
        .mf { display: flex; justify-content: space-between; align-items: flex-end; font-size: ${8 * scale}px; flex-grow: 1; min-height: 0; gap: ${4 * scale}px; }
        .bc img { width: 100%; max-height: ${6 * scale}mm; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="mb">
           <div class="mh">
           ...
           <div class="mf">
              <div class="bc" style="flex: 1;"><img src="${barcode}" /></div>
              ${expiry ? `<div style="text-align:right; flex-shrink: 0;">Exp: ${expiry}</div>` : ""}
           </div>
        </div>
        ${BRANDING_HTML}
      </div>
  ...
  med_expiry: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-weight: 700; font-size: ${10 * scale}px; line-height: 1.1; overflow: hidden; width: 100%; }
        .dt { font-size: ${8 * scale}px; font-weight: 700; color: #d32f2f; }
        .pr { font-size: ${14 * scale}px; font-weight: 800; }
        .ex-l img { width: 100%; height: ${6 * scale}mm; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="ex-box">
           <div class="ex-l">
              <div class="nm">${nameHTML}</div>
              <img src="${barcode}" style="margin: 2px 0 0 0;" />
           </div>
           <div class="ex-r">
  ...
  med_strip: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-weight: 700; font-size: ${9 * scale}px; overflow: hidden; width: 100%; }
  ...
  med_rack: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-size: ${9 * scale}px; font-weight: 600; line-height: 1.1; width: 100%; }
        .bc { flex-grow: 1; display: flex; align-items: center; justify-content: center; min-height: 0; width: 100%; }
        .bc img { width: 100%; max-height: 100%; object-fit: fill; }
      </style>
      <div class="wrapper">
        <div class="rk">
           ...
           <div class="rk-b">
              <div class="nm">${nameHTML}</div>
              ${item.storage_location ? `<div style="font-size:${8 * scale}px;">Loc: ${item.storage_location}</div>` : ""}
              <div class="bc"><img src="${barcode}" /></div>
           </div>
        </div>
        ${BRANDING_HTML}
  ...
  med_generic: (item, shop, barcode, width, height = 25) => {
    ...
    return `
      <style>
        ...
        .nm { font-weight: 700; font-size: ${10 * scale}px; color: #0066cc; line-height: 1.1; width: 100%; }
