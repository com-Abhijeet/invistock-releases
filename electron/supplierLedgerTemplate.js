/**
 * Generates the HTML for a Supplier Statement of Account.
 * @param {object} shop - Shop details.
 * @param {object} supplier - Supplier details.
 *_ @param {Array<object>} ledger - The nested array of purchases and transactions.
 * @returns {string} The complete HTML string.
 */
function createSupplierLedgerHTML(shop, supplier, ledger) {
  const style = `
    <style>
      ...
      .supplier-details p { margin: 3px 0; }
  ...
  const html = `
    <!DOCTYPE html>
    <html><head><title>Supplier Ledger</title>${style}</head>
    <body>
      <div class="container">
        <div class="header">
          <h2>${shop.shop_name}</h2>
          <p>${shop.address_line1 || ""}, ${shop.city || ""}</p>
          <h3>Statement of Account (Supplier)</h3>
        </div>
        <div class="divider"></div>
        <div class="supplier-details">
          <p><strong>Supplier:</strong> ${supplier.name}</p>
          <p><strong>Phone:</strong> ${supplier.phone || "N/A"}</p>
          <p><strong>Address:</strong> ${[supplier.address, supplier.city]
            .filter(Boolean)
            .join(", ")}</p>
          ${
            supplier.gst_number
              ? `<p><strong>GSTIN:</strong> ${supplier.gst_number}</p>`
              : ""
          }
        </div>
        <table class="summary-table">
          ...
          <tfoot>
            <tr style="border-top: 2px solid #333; font-size: 1.2em; font-weight: bold;">
              <td colspan="4" class="right">Total Payable Balance:</td>
...
module.exports = { createSupplierLedgerHTML };
